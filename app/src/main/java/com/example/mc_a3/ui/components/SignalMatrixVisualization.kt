...
fun SignalMatrixVisualization(
    ...
    // Check if the matrix is empty (indicating no networks found)
    val noNetworksFound = signalMatrix.isEmpty()
    ...
    Column(modifier = modifier) {
        ...
        if (noNetworksFound) {
            // Display a message when no networks are found
            Box(
            ...
            // Display the signal strength visualization
            Canvas(
                modifier = Modifier
                    .fillMaxWidth()
                    .height(200.dp)
